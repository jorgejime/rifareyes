...
const InfoRow: React.FC<{ label: string; value: React.ReactNode; className?: string }> = ({ label, value, className = '' }) => (
  <div className={`flex justify-between items-center py-2 border-b border-slate-600/50 ${className}`}>
    ...
    <span className="font-bold text-slate-100 text-lg text-right">{value}</span>
  </div>
);
  ...
  if (showConfirmation) {
    return (
      <div className="animate-fade-in text-center flex flex-col justify-center h-full">
        ...
        <p className="text-slate-300 mt-2">Tus números han sido apartados. Tienes <span className="font-bold text-yellow-300">30 minutos</span> para completar el pago y asegurar tu participación.</p>
        
        <div className="mt-4 p-3 bg-slate-900/50 rounded-lg border border-slate-700 text-left space-y-2">
            <p className="text-sm text-slate-300">1. Paga <span className="font-bold text-white">${total.toLocaleString('es-CO')}</span> a nuestra cuenta Nequi:</p>
            <p className="text-2xl font-black text-yellow-400 text-center tracking-wider">{NEQUI_NUMBER}</p>
            <p className="text-sm text-slate-300 mt-1">2. Envía el comprobante por WhatsApp para validar tu compra.</p>
        </div>
        ...
        >
         ...
         <button onClick={onResetPurchase} className="w-full text-center mt-2 text-sm text-slate-400 hover:text-yellow-400">Seleccionar otros números</button>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <button
          ...
          className={`block w-full text-center mt-4 py-3 px-6 text-white font-bold text-lg rounded-lg shadow-lg transition-all duration-200 ${isFormValid ? 'bg-gradient-to-r from-teal-500 to-cyan-600 hover:scale-105 active:scale-100' : 'bg-slate-700 opacity-60 cursor-not-allowed'}`}
          aria-disabled={!isFormValid}
      >
        {isFormValid ? 'Reservar Números' : 'Completa tus datos'}
      </button>
      <button onClick={onResetPurchase} className="w-full text-center mt-2 text-sm text-slate-400 hover:text-red-400">Cancelar Selección</button>
    </div>
  );
};
  ...
  const renderContent = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-48">
        <p className="text-slate-300 text-center">Selecciona uno o más números del tablero para iniciar tu compra.</p>
      </div>
    );
  };

  return (
    <div className="w-full max-w-lg mx-auto text-left p-6 bg-slate-800/60 backdrop-blur-sm border border-teal-500/40 rounded-2xl shadow-2xl min-h-[300px] flex flex-col">
